...
function report_row(&$row, $sortby)
{
  ...
  // Materiau
  //$values[] = escape($row['materiau_name']);
  // Start date
  if ($enable_periods)
  {
  ...
  $values[] = escape($row['prenom']. " " .$row['nom']);
...
//$materiaumatch = get_form_var('materiaumatch', 'string');
...
if ($phase == 2)
{
  ...
  $sql = "SELECT E.*, "
       ...
       . "A.approval_enabled, A.confirmation_enabled, A.enable_periods, U.prenom, U.nom";
  if ($output_as_ical)
  {
  ...
  $sql .= " FROM $tbl_area A, $tbl_room R, $tbl_entry E, $tbl_users U";
  if ($output_as_ical)
  {
...
/*  if (!empty($materiaumatch))
  {
    // sql_syntax_caseless_contains() does the SQL escaping
    $sql .= " AND" .  sql_syntax_caseless_contains("N.materiau_name", $materiaumatch);
  }*/
  if (!empty($typematch))
  {
    $sql .= " AND ";
      ...
      <legend><?php echo get_vocab("report_on");?></legend>
        ...
        <div id="div_sumby">
          ...
          <div class="group">
            ...
            <label>
              <input class="radio" type="radio" name="sumby" value="p"
              <?php 
              if ($sumby=="p") echo " checked=\"checked\"";
              echo ">" . get_vocab("sum_by_projet");
              ?>
            </label>
          </div>
        </div>
