...
$admin_id = get_form_var('new_admin', 'string');
$coadmin_id = get_form_var('new_coadmin', 'string');
$room_admin_email = get_form_var('room_admin_email', 'string');
$room_users_email = get_form_var('room_users_email', 'string');
...
$print_admin = get_form_var('print_admin', 'string');
...
if (isset($print_admin))
{
  if (!empty($room)) // Get the area the room is in
  {
    $area = mrbsGetRoomArea($room);
  }
  //Header("Location: generate_sheet.php?equipment=$room");
  //exit();
...
if ($phase == 2)
{
  ...
  if (isset($change_room) && !empty($room))
  {
    ...
    $valid_email = validate_email_list($room_users_email);  
          ...
          if ($field['name'] != 'id')  // don't do anything with the id field
          {
            switch ($field['name'])
            {
              ...
              case 'room_admin_email':
                ...
                // If no admin email is specified, we create a default adress
                if ($room_admin_email == ""){
                    list($admin_mail, $coadmin_mail) = roomadmin_adress2($admin_id, $coadmin_id);
                    $room_admin_email = $admin_mail . ', ' . $coadmin_mail;
                ...
                break;
              case 'admin_id':
                $assign_array[] = "admin_id='" . addslashes($admin_id) . "'";
                break; 
              case 'coadmin_id':
                $assign_array[] = "coadmin_id='" . addslashes($coadmin_id) . "'";
                break; 
              ...
              default:
                ...
                if ($field['name'] == "room_users_email")
                {
                    $$var = clean_address_list($$var);
                    $$var = str_replace(',', ', ', $$var);
                }
...
//      }
    ...
    $sql = "SELECT id, prenom, nom FROM $tbl_users ORDER BY nom";
    $res = sql_query($sql);
      if (!$res)
      {
        trigger_error(sql_error(), E_USER_WARNING);
      ...
      if (sql_count($res) == 0)
      {
        fatal_error(FALSE, get_vocab('nousers'));  // should not happen
      }
    
    // The admin select box
      echo "<div>\n";
      echo "<label for=\"new_admin\">" . get_vocab("room_admin_name") . ":</label>\n";
      echo "<select id=\"new_admin\" name=\"new_admin\"$disabled>\n";
        for ($i = 0; ($row_admin = sql_row_keyed($res, $i)); $i++)
        {
          echo "<option value=\"" . $row_admin['id'] . "\"";
          if ($row_admin['id'] == $row['admin_id'])
          {
            echo " selected=\"selected\"";
          }
          echo ">" . htmlspecialchars($row_admin['prenom'] . ' ' . $row_admin['nom']) . "</option>\n";
        }  
      echo "</select>\n";
      $htlm_name = get_vocab("print_admin");
      echo "<a title=\"$htlm_name\" target = \'_blank\' href=generate_sheet.php?equipment=$room >" . $htlm_name . "</a>";
      echo "</div>\n";
    ...
    // The coadmin select box
      echo "<div>\n";
      echo "<label for=\"new_coadmin\">" . get_vocab("room_coadmin_name") . ":</label>\n";
      echo "<select id=\"new_coadmin\" name=\"new_coadmin\"$disabled>\n";
        for ($i = 0; ($row_coadmin = sql_row_keyed($res, $i)); $i++)
        {
          echo "<option value=\"" . $row_coadmin['id'] . "\"";
          if ($row_coadmin['id'] == $row['coadmin_id'])
          {
            echo " selected=\"selected\"";
          }
          echo ">" . htmlspecialchars($row_coadmin['prenom'] . ' ' . $row_coadmin['nom']) . "</option>\n";
        }  
      echo "</select>\n";
      echo "</div>\n";
...
//  echo "</div>\n";
  ...
  /* // LISTE DES MATERIAUX D'UN EQUIPEMENT
      
// BOTTOM SECTION: materiau IN THE SELECTED EQUIPEMENT
...
if ($is_admin)
{
  ...
  echo "<h2>\n";
  echo get_vocab("materiaux");
  if(isset($room_name))
  { 
    echo " " . get_vocab("in") . " " . htmlspecialchars($room_name); 
  }
  ...
  if (isset($room))
  {
    ...
    $res = sql_query("SELECT * FROM $tbl_materiau WHERE room_id=$room ORDER BY sort_key3");
    if (! $res)
    {
      trigger_error(sql_error(), E_USER_WARNING);
      fatal_error(FALSE, get_vocab("fatal_db_error"));
    ...
    if (sql_count($res) == 0)
    {
      echo "<p>" . get_vocab("nomateriau") . "</p>\n";
    }
    else
    {
       // Get the information about the fields in the materiau table
      $fields = sql_field_info($tbl_materiau);

      // Build an array with the materiau info and also see if there are going
      // to be any materiau to display (in other words materiau if you are not an
      // admin whether any materiaux are enabled)
      $materiaux = array();
      $n_displayable_materiaux = 0;
      for ($i = 0; ($row = sql_row_keyed($res, $i)); $i++)
      {
        $materiaux[] = $row;
        if ($is_admin || !$row['disabled'])
        {
          $n_displayable_materiaux++;
        }
      }

      if ($n_displayable_materiaux == 0)
      {
        echo "<p>" . get_vocab("nomateriaux_enabled") . "</p>\n";
      }
      else
      {
        echo "<div id=\"materiau_info\" class=\"datatable_container2\">\n";
        ...
        // database)
        echo "<table id=\"materiau_table\" class=\"admin_table display\">\n";
        ...
        if ($is_admin)
        {
        // Don't show ordinary users the disabled status:  they are only going to see enabled materiaux
        ...
        $ignore = array('id', 'room_id', 'materiau_name', 'disabled', 'sort_key3', 'custom_html');
        foreach($fields as $field)
        {
          if (!in_array($field['name'], $ignore))
          {
            ...
            switch ($field['name'])
            {
              ...
              default:
                $text = get_loc_field_name($tbl_materiau, $field['name']);
                break;
            }
        ...
        echo "<tbody>\n";
        $row_class = "odd";
        foreach ($materiaux as $r)
        {
          // Don't show ordinary users disabled materiaux
          if ($is_admin || !$r['disabled'])
          {
            $row_class = ($row_class == "even") ? "odd" : "even";
            echo "<tr class=\"$row_class\">\n";

            $html_name = htmlspecialchars($r['materiau_name']);
            echo "<td><div><a title=\"$html_name\" href=\"edit_materiau.php?change_materiau=1&amp;phase=1&amp;room=" . $room ."&amp;materiau=" . $r['id']. ";\">$html_name</a></div></td>\n";
            if ($is_admin)
            {
              // Don't show ordinary users the disabled status:  they are only going to see enabled materiaux
            ...
            if ($is_admin)
            {
              // Delete link
              echo "<td><div>\n";
              echo "<a href=\"del.php?type=materiau&amp;room=$room&amp;materiau=" . $r['id'] . "\">\n";
  ...
  else
  {
    ...
    /*      // Give admins a form for adding materiau to the equipment - provided 
  // there's an equipment selected
  if ($is_admin && !empty($area))
  {
  ?>
    <form id="add_room" class="form_admin" action="add.php" method="post">
      <fieldset>
      <legend><?php echo get_vocab("addmateriau") ?></legend>
        
        <input type="hidden" name="type" value="materiau">
        <input type="hidden" name="room" value="<?php echo $room; ?>">
        
        <div>
          <label for="materiau_name"><?php echo get_vocab("name") ?>:</label>
          <input type="text" id="materiau_name" name="name" maxlength="<?php echo $maxlength['room.room_name'] ?>">
        </div>
        
        <div>
          <label for="materiau_description"><?php echo get_vocab("description") ?>:</label>
          <input type="text" id="materiau_description" name="description" maxlength="<?php echo $maxlength['room.description'] ?>">
        </div>
        
        <div>
          <label for="materiau_email"><?php echo get_vocab("materiau_admin_email") ?>:</label>
          <input type="text" id="materiau_email" name="email">
        </div>
       
        <div>
          <input type="submit" class="submit" value="<?php echo get_vocab("addmateriau") ?>">
        </div>
