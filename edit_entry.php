...
function create_field_entry_areas($disabled=FALSE)
{
  ...
  { 
    ?> 
      ...
      //  changeMateriaux2(document.formulaire.rooms.value, document.formulaire.rooms.form);
      }
...
/*function create_field_entry_materiaux($disabled=FALSE)
{
  global $materiaux, $multimateriau_allowed, $materiau_id, $room_id, $selected_materiaux, $original_materiau;

  echo "<div id=\"div_materiaux\">\n";
  echo "<label for=\"materiaux\">" . get_vocab("materiaux") . ":</label>\n";
  echo "<div class=\"group\">\n";
  echo "<select id=\"materiaux\" name=\"materiaux[]\"" .
    (($multimateriau_allowed) ? " multiple=\"multiple\"" : "") .
    (($disabled) ? " disabled=\"disabled\"" : "") .
    " size=\"5\" >\n";
  // $selected_materiaux will be populated if we've come from a drag selection
  if (empty($selected_materiaux))
  {
    $selected_materiaux = array($materiau_id);
  }
  foreach ($materiaux as $r)
  {
    if ($r['room_id'] == $room_id)
    {
      $is_selected = in_array($r['id'], $selected_materiaux);
      $selected = ($is_selected) ? "selected=\"selected\"" : "";
      echo "<option $selected value=\"" . $r['id'] . "\">" . htmlspecialchars($r['materiau_name']) . "</option>\n";
    }
  }
  echo "</select>\n";
  // No point telling them how to select multiple materiaux if the input
  // is disabled
    $multimateriau = false;
  if ($multimateriau_allowed && !$disabled)
  {
    echo "<span>" . get_vocab("ctrl_click") . "</span>\n";
  }
  echo "</div>\n";
  if ($disabled)
  {
      print_r($selected_materiaux);
    foreach ($selected_materiaux as $selected_materiau)
    {
      echo "<input type=\"hidden\" name=\"materiaux[]\" value=\"$selected_materiau\">\n";
    }
  }
  echo "</div>\n";
    ...
    ?>
    <script type="text/javascript">
        // changeMateriaux2(document.formulaire.rooms.value, document.formulaire.rooms.form, <?php // echo $original_materiau ?>);
        changeMateriaux2(document.formulaire.rooms.value, document.formulaire.rooms.form, document.getElementById("div_rooms").value);
        //document.getElementById("div_rooms").onchange();
        //document.getElementById("div_rooms").click();
        //document.getElementById("div_rooms").focus();
    </script>
...
function create_field_entry_type($user, $room_id, $disabled=FALSE)
{
  ...
  {
      // Don't put type E (maintenance) if the user
      // is not allowed to book one
      if ($key == "E" && user_can_maintenance($room_id, $user) 
          || $key != "E"){
...
function create_field_entry_custom_field($field, $key, $disabled=FALSE)
{
  ...
  {
    // For the "finished" field, we don't allow it to be edited
    // if it was already put as true
      
    echo "<label for=\"$var_name\">$label_text</label>\n";
    echo "<input type=\"checkbox\" class=\"checkbox\" " .
      "id=\"$var_name\" name=\"$var_name\" value=\"1\" " .
      ((!empty($value)) ? " checked=\"checked\"" : "") .
      (($disabled || $var_name == "f_finished" && !empty($value)) ? " disabled=\"disabled\"" : "") .
      (($var_name == "f_finished") ? " onclick=\"updateFinished()\"" : "") .   
       ">\n";
  }
  ...
  if ($var_name == 'projet')
  {
      echo "<input type=\"hidden\" name=\"projet\" value=\"$value\" autocomplete=\"off\" onkeyup=\"refresh(this.value)\">\n";
  }
  else{
    if ($disabled)
    {
    ...
    // if the finished is checked, then the value would not be passed. So,
    // we add the hidden field with the same name and value to pass the value
  if ($var_name == "f_finished" && !empty($value)){
      echo "<input type=\"hidden\" name=\"$var_name\" value=\"$value\">\n";
  }
  echo "</div>\n";
...
if (isset($id))
{
  ...
  foreach ($row as $column => $value)
  {
    switch ($column)
    {
      ...
      case 'materiau_id':    
        $original_materiau=$row['materiau_id'];
...
/*// Get the details of all the enabled materiaux
$materiaux = array();
$sql = "SELECT M.id, M.materiau_name, M.room_id
          FROM $tbl_materiau M, $tbl_room R
         WHERE M.room_id = R.id
           AND R.disabled=0
      ORDER BY M.room_id, M.sort_key";
$res = sql_query($sql);
if ($res)
{
  for ($i = 0; ($row = sql_row_keyed($res, $i)); $i++)
  {
    $materiaux[$row['id']] = $row;
  }
}*/

// Get the details of all the projet
$projet = array();
$sql = "SELECT projet
          FROM $tbl_entry
      ORDER BY projet";
...
function validate(form_id)
{
  ...
  if (form.elements['rooms'].selectedIndex == -1 )
  {
   ...
   if (form.elements['materiaux'].selectedIndex == -1 )
  {
    alert("<?php echo escape_js(get_vocab('you_have_not_selected')) . '\n' . escape_js(get_vocab('valid_materiau')) ?>");
    return false;
  }
...
<form class="form_general" id="main" name="formulaire" action="edit_entry_handler.php" method="post">
  <fieldset>
...
foreach( $edit_entry_field_order as $key )
{
  ...
  case 'type':
    create_field_entry_type($user, $room_id);
    break;
    ...
    <script type="text/javascript">
       $(function() {
            $('.radio').change(function() {
                ...
                //$('.rep_end_date').hide();    
                $('.rep_day').hide('slow');
                $('.rep_num_weeks').hide('slow');   
                switch ($val) 
                {
                        case '1' : 
                            $('.rep_end_date').show('slow');
                            break;
                        case '2' :
                            $('.rep_end_date').show('slow');    
                            $('.rep_day').show('slow');
                        ...
                        case '6' :
                            $('.rep_end_date').show('slow');    
                            $('.rep_day').show('slow');
                            $('.rep_num_weeks').show('slow');
                            break;
                        case '0' : 
                            $('.rep_end_date').hide('slow');    
                            $('.rep_day').hide('slow');
                            $('.rep_num_weeks').hide('slow');   
                            break;
                }
      ...
      <div id="rep_end_date" class="rep_end_date">
        <?php
      ...
      echo "<div>\n";
      echo "<label for=\"skip\" style=\"display:none\">" . get_vocab("skip_conflicts") . ":</label>\n";
      echo "<input type=\"checkbox\" class=\"checkbox\" " .
