...
if ($is_admin)
{
  // New area form
  ?>
  <form id="add_area" class="form_admin" action="add.php" method="post" autocomplete="off">
    <fieldset>
...
if ($is_admin || ($n_displayable_areas > 0))
{
  ...
  {
  ?>
    <form id="add_room" class="form_admin" action="add.php" method="post" autocomplete="off">
      <fieldset>
      <legend><?php echo get_vocab("addroom") ?></legend>
        ...
        <div>
          <label for="room_email"><?php echo get_vocab("room_admin_email") ?>:</label>
          <input type="text" id="room_email" name="email">
        </div>
    ...
    else
    {
      ...
      else
      {
        ...
        // The body
          ...
          ///////////////////////////////////
       //We create an array which associates id with prenom + nom    
        $res = sql_query("SELECT id, prenom, nom FROM $tbl_users");
      if (!$res)
      {
        trigger_error(sql_error(), E_USER_WARNING);
        fatal_error(FALSE, get_vocab("fatal_db_error"));
      ...
      $name_array = array();
      for ($i = 0; ($row_name = sql_row_keyed($res, $i)); $i++)
        {
            $name_array[$row_name['id']] = htmlspecialchars($row_name['prenom'] . ' ' . $row_name['nom']);
      }   
        /////////////////////////////////   
          ...
          if ($is_admin || !$r['disabled'])
          {
            ...
            foreach($fields as $field)
            {
              if (!in_array($field['name'], $ignore))
              {
                ...
                case 'admin_id':
                    case 'coadmin_id':
                    echo "<td><div>" . htmlspecialchars($name_array[$r[$field['name']]]) . "</div></td>\n";
                    break;
