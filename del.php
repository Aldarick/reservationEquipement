...
$materiau = get_form_var('materiau', 'string');
...
/*if ($type == "materiau")
{
  ...
  if (isset($confirm))
  {
    ...
    sql_begin();
    // First take out all appointments for this room
    sql_command("delete from $tbl_entry where materiau_id=$materiau");
    sql_command("delete from $tbl_repeat where materiau_id=$materiau");
   
    // Now take out the room itself
    sql_command("delete from $tbl_materiau where id=$materiau");
    sql_commit();
  ...
  else
  {
    ...
    $sql = "select name, start_time, end_time from $tbl_entry where materiau_id=$materiau";
    ...
    echo "<p>" .  get_vocab("sure") . "</p>\n";
    echo "<div id=\"del_room_confirm_links\">\n";
    echo "<a href=\"del.php?type=materiau&amp;room=$room&amp;materiau=$materiau&amp;confirm=Y\"><span id=\"del_yes\">" . get_vocab("YES") . "!</span></a>\n";
    echo "<a href=\"admin.php\"><span id=\"del_no\">" . get_vocab("NO") . "!</span></a>\n";
    echo "</div>\n";
    echo "</div>\n";
    require_once "trailer.inc";
